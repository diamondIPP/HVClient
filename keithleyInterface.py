    ...
    def read(self,minLength=1):
        ...
        if not self.bOpen:
            if not self.bOpenInformed:
                ...
                self.bOpenInformed = False
            return ''
        while self.serial.inWaiting()<=0 and i<20:
        ...
        finished =False
        #while len(out)<minLength and time.time()-ts<maxTime and not finished:
        while  time.time()-ts<maxTime and not finished:
            while self.serial.inWaiting() > 0 and time.time()-ts<maxTime and not finished:
                char = self.serial.read(1)
                out +=char
                if ord(char) == self.terminator:
                    finished = True
                    #print 'finished'
                #print out
                if char in self.removeCharacters:
                    #print ord(char)
                    pass
...
#        print printVal
    ...
    def getOutputStatus(self):
        ...
        ##print 'length ',len(answer),ord(answer[-1]),answer[-1],answer,
        #while len(answer)>2 and not self.is_number(answer):
        #    time.sleep(.1)
        #    answer= self.getAnswerForQuery(data)
        #print answer,self.serial.inWaiting()
